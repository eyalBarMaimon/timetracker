...
async function dropboxSync(mode) {
  ...
  try {
    if (mode === 'read') {
      const resp = await dropboxClient.filesDownload({ path: '/ClaudeCode/Time Track/timetracker_data.json' });
      ...
      await dropboxClient.filesUpload({
        path: '/ClaudeCode/Time Track/timetracker_data.json',
        contents: content,
        mode: { '.tag': 'overwrite' }
      });
    }
    ...
    if (e && e.status === 409) {
      ...
      try {
        const content = JSON.stringify(data, null, 2);
        await dropboxClient.filesUpload({ path: '/ClaudeCode/Time Track/timetracker_data.json', contents: content, mode: { '.tag': 'add' } });
        setSyncStatus('ok');
